...
const NETWORK_FLOW_STEPS = [
  {
    ...
    title: "WAN / ISP",
    sub: "Carrier handoff",
    hint: "Untrusted ingress: BGP or static default route from the ISP; demarc optics (e.g. 10GE LR).",
  },
  {
    key: "fw",
    Icon: Shield,
    title: "NGFW",
    sub: "Policy & zoning",
    hint: "Perimeter firewall: application policy, VLAN tagging into the trusted core.",
  },
  {
    key: "core",
    Icon: Layers,
    title: "Core L3",
    sub: "SVI & routing",
    hint: "Agg switch / router: terminates VLAN SVIs or downlinks trunked to distribution / access switches.",
  },
  {
    key: "ids",
    Icon: Radar,
    title: "IDS / SPAN",
    sub: "Mirror ingest",
    hint: "Packet mirror (SPAN/ERSPAN) from the NGFW interior port into the sensor VM or appliance.",
  },
  {
    key: "vlan",
    Icon: GitBranch,
    title: "ToR access",
    sub: "L2 per zone",
    hint: "Top-of-rack or closet switch serving one broadcast domain mapped to VLAN + SVI subnet.",
  },
  {
    key: "iot",
    Icon: Server,
    title: "Hosts",
    sub: "ICS / IoT",
    hint: "Field devices, sensors, or fog/compute blades with host routes via the gateway on the VLAN.",
  },
];
  ...
  return [
    {
      ...
      body: "Dim or coloured lines with arrows show the logical path: untrusted → firewall → core (packets mirrored or analysed by IDS) → zone VLAN switch → host. The bright dashed arrow that fades after ~5s is the same live event.",
    },
    {
      title: "What does this alert mean?",
      body: `${alert.attack_type} — activity between ${alert.src_ip} → ${alert.dst_ip}${
...
function buildNodes(topoData, pulsing) {
  ...
  const vlanSpan = sortedVlans.length
    ? (sortedVlans.length - 1) * VLAN_X_STEP
    : 0;
  ...
  sortedVlans.forEach((vlan, idx) => {
    ...
    const innerBlock = Math.max(
      DEVICE_X_STEP,
      (devicesInVlan.length - 1) * DEVICE_X_STEP + 104,
    );
  ...
  return topoData.edges.map((e) => {
    ...
    return {
      ...
      data: {
        ...
        label: e.label,
        kind,
        color: e.data?.color ?? (kind === "trunk" ? "#65758b" : "#4b596d"),
      },
    };
  });
}
...
export default function Topology({ user }) {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(
      `${wsProtocol}//${wsHost}/ws/alerts?token=${encodeURIComponent(token)}`,
    );
  ...
  const toggleRule = async (rule) => {
    try {
      await api.patch(`/api/network/firewall/${rule.id}`, {
  ...
  return (
    <div
      className="page-enter flex h-[calc(100vh-80px)] gap-4"
      lang="en"
      dir="ltr"
    >
      {/* ── React Flow canvas ─────────────────────────────────────────── */}
      <div className="flex-1 bg-card border border-border rounded-2xl overflow-hidden relative">
        <div className="absolute top-4 left-4 z-10 flex flex-col items-start gap-2 max-w-md">
          ...
          {legendOpen && (
            <div className="text-[10px] leading-relaxed text-text/80 bg-background/95 border border-border rounded-xl p-3 shadow-lg max-h-64 overflow-y-auto space-y-2">
              <p className="font-bold text-accent">Network layers</p>
              <p className="text-text/70">
                Internet → city firewall → core → control plane &amp; IDS engine
                → per-zone VLAN switches → hosts in each segment (data from{" "}
                <code className="text-accent/90 bg-card px-0.5 rounded">
              ...
              <ul className="list-disc space-y-1 ps-4 text-text/70">
                <li>
                  <strong>WAN handoff block</strong> plus rack-face NGFW, core
                  agg, IDS, and ToR nodes—hostnames/models/mgmt IPs and VLAN
                  gateways mimic a NetBox lab export.
                </li>
                <li>
                  Dashed <strong>traffic</strong> edges annotate speed plus
                  interface shorthand (carrier demarc → Po uplinks → Gi access →
                  NIC). Alerts still originate from{" "}
                  <code className="text-accent/90 bg-card px-0.5 rounded">
                    /ws/alerts
                  </code>{" "}
                  (~5s glow between hosts).
                </li>
              </ul>
              <p className="font-bold text-accent">How it works</p>
              <p className="text-text/70">
                IDS records the event → SQLite → broadcast to clients → animated
                arrow between source and target; Live Logs tab shows the same
                JSON.
              </p>
              <p className="text-text/70">
                Firewall tab:{" "}
          ...
          <div className="absolute bottom-[148px] left-3 right-3 z-[8] pointer-events-none flex justify-center px-2">
            <div className="pointer-events-auto max-w-2xl w-full rounded-2xl border border-high/35 bg-background/96 p-4 shadow-2xl backdrop-blur-md">
              <div className="flex items-start gap-3">
                <Zap
                  className="mt-0.5 h-5 w-5 shrink-0 text-high drop-shadow-[0_0_8px_rgba(239,68,68,0.55)]"
                  aria-hidden
                />
                <div className="min-w-0 flex-1 space-y-3 text-left">
                  <div>
                    <p className="text-[11px] font-bold text-accent">
                      How this attack appears on the topology
                    </p>
                    <p className="mt-0.5 truncate text-[9px] font-mono text-text/55">
                      [{attackStory.alert.severity}]{" "}
                      {attackStory.alert.attack_type}
                    </p>
                  </div>
        ...
        {!loading && (
          <div className="absolute bottom-3 left-2 right-2 z-[7] pointer-events-none flex justify-center px-2">
            <div className="pointer-events-auto max-w-[98%] overflow-x-auto rounded-2xl border border-border bg-background/96 px-3 py-2.5 shadow-xl backdrop-blur-sm sm:px-4 sm:py-3">
              <p className="mb-2 text-center text-[11px] font-bold text-accent">
                Physical-ish north–south path (left → right)
              </p>
              <div className="flex flex-nowrap items-stretch justify-center gap-0.5 min-w-min sm:gap-1">
                {NETWORK_FLOW_STEPS.map((s, i) => (
                  <React.Fragment key={s.key}>
                    {i > 0 && (
                      <ChevronRight
                        className="h-4 w-4 shrink-0 self-center text-accent/40 sm:h-5 sm:w-5"
                        aria-hidden
                      />
                    )}
                    <HoverHint
                    ...
                    >
                      <s.Icon
                        className="h-[18px] w-[18px] shrink-0 text-sky-400/90 sm:h-[20px] sm:w-[20px]"
                        strokeWidth={1.85}
                        aria-hidden
                      />
                      ...
                      <span className="text-center text-[6.5px] leading-snug text-slate-500">
                        {s.sub}
                      </span>
                    </HoverHint>
                  </React.Fragment>
              ...
              <p className="mx-auto mt-2 max-w-lg text-center text-[8px] leading-relaxed text-text/45">
                Edge labels pulled from topology JSON (circuit, SPAN, VLAN,
                access port → NIC). Alerts still draw live device-to-device
                overlays from WebSocket.
              </p>
            </div>
          </div>
        )}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              ...
              {tab === "Hosts" && (
                <div className="space-y-3">
                  <p className="text-[10px] leading-relaxed text-text/55">
                    Device will appear under the selected VLAN zone. IP must be
                    within that zone's subnet range.
                  </p>
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-text/60 uppercase">
                      Network Devices
                    </span>
                    {canManageDevices && (
                      <button
                  ...
                  {addingDevice && canManageDevices && (
                    <div className="p-3 bg-background border border-accent/20 rounded-xl space-y-2">
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="text-[9px] text-text/50 w-16 shrink-0">
                          Zone
                        </span>
                        <select
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex items-center gap-1.5 min-w-0">
                              <span
                                className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${d.status === "online" ? "bg-green-400" : "bg-red-400"}`}
                              />
                              <span className="font-mono text-sky-300/90 truncate">
                                {d.ip}
                              </span>
                            </div>
                            <span
                              className={`text-[9px] font-bold px-1 py-0.5 rounded flex-shrink-0 ${d.status === "online" ? "text-green-400" : "text-red-400"}`}
                            >
                          ...
                          {d.mac && (
                            <p className="text-[9px] text-text/25 font-mono mt-0.5">
                              {d.mac}
                            </p>
                          )}
                        </div>
              ...
              {tab === "Firewall" && (
                <div className="space-y-3">
                  ...
                  {addingRule && user?.role === "admin" && (
                    <div className="p-3 bg-background border border-accent/20 rounded-xl space-y-2">
                      ...
                      <div className="flex justify-end space-x-2 pt-1">
                        ...
                        <button
                          onClick={addRule}
                          className="text-[10px] px-3 py-1.5 bg-accent text-white rounded-lg font-bold hover:bg-accent/90 transition-all"
                        >
                          Save
                        </button>
                      </div>
                  ...
                  {/* Rules list */}
                  {fwLoading ? (
                    <div className="text-center text-text/30 text-xs py-4">
                      Loading…
                    </div>
                  ) : (
                    fwRules.map((rule) => (
                      ...
                      >
                            ...
                            {user?.role === "admin" && (
                              <>
                                <HoverHint
                                  ...
                                  hint={
                                    rule.enabled
                                      ? "Disable this firewall rule (stops enforcement)."
                        ...
                        {rule.description && (
                          <div className="text-[9px] text-text/40 mt-0.5">
                            {rule.description}
                          </div>
                        )}
                      </div>
              ...
              {tab === "Encryption" && (
                ...
                >
                  ...
                  <div
                    style={{
                      background:
                        "linear-gradient(135deg,rgba(0,255,157,.09),rgba(0,229,255,.05))",
                    ...
                    onMouseEnter={(e) =>
                      (e.currentTarget.style.boxShadow =
                        "0 0 32px rgba(0,255,157,.16)")
                    }
                    onMouseLeave={(e) =>
                      (e.currentTarget.style.boxShadow =
                        "0 0 20px rgba(0,255,157,.08)")
                    }
                  >
                    <div
                      style={{
                    ...
                    <div
                      style={{
                        ...
                        borderRadius: 20,
                        background:
                          "linear-gradient(90deg,rgba(0,229,255,.1),rgba(157,92,255,.12),rgba(0,229,255,.1))",
                  ...
                  <div
                    ...
                    onMouseEnter={(e) => {
                      e.currentTarget.style.borderColor = "rgba(0,229,255,.28)";
                      e.currentTarget.style.boxShadow =
                        "0 0 32px rgba(0,229,255,.08)";
                    }}
                    onMouseLeave={(e) => {
                      e.currentTarget.style.borderColor = "rgba(0,229,255,.14)";
                      e.currentTarget.style.boxShadow =
                  ...
                  >
                    {[
                      ...
                      {
                        label: "Security Key",
                        value: "A7F3C91E5B2D8A4F6C1E9B3D7F2A8C5E",
                        color: "#9D5CFF",
                        mono: true,
                        hidden: true,
                      },
                      {
                        label: "Key Format",
                        value: "256-bit Base64 (Fernet)",
                        color: "rgba(123,145,176,.8)",
                        mono: false,
                      },
                      {
                        label: "Active Model",
                        value: config.active_model || "RandomForest",
                        color: "#00FF9D",
                        mono: false,
                      },
                      {
                        ...
                        value: config.anomaly_threshold
                          ? `${parseFloat(config.anomaly_threshold).toFixed(1)}%`
                          : "55.0%",
                        color: "#00E5FF",
                        mono: false,
                      },
                      ...
                      },
                      {
                        ...
                        value:
                          config.last_tuned && config.last_tuned !== "Never"
                            ? new Date(config.last_tuned).toLocaleString(
                                "en-US",
                              )
                            : "Pending",
                        color: "rgba(123,145,176,.8)",
                      ...
                      const displayVal =
                        hidden && !keyRevealed ? "•".repeat(16) : value;
                      return (
                        <div
                          key={label}
                          style={{
                          ...
                          >
                            <span
                              style={{
                                ...
                                filter:
                                  mono && !hidden
                                    ? `drop-shadow(0 0 5px rgba(157,92,255,.4))`
                                    : "none",
                              }}
                            >
                              {displayVal}
                  ...
                  <div
                    style={{
                      ...
                      boxShadow:
                        "inset 0 1px 0 rgba(0,229,255,.1), 0 8px 32px rgba(0,0,0,.4)",
                      transition: "border-color .3s ease, box-shadow .3s ease",
                      position: "relative",
                      overflow: "hidden",
                    }}
                    onMouseEnter={(e) => {
                      e.currentTarget.style.borderColor = "rgba(0,229,255,.32)";
                      e.currentTarget.style.boxShadow =
                        "inset 0 1px 0 rgba(0,229,255,.18), 0 0 28px rgba(0,229,255,.07)";
                    }}
                    onMouseLeave={(e) => {
                      e.currentTarget.style.borderColor = "rgba(0,229,255,.18)";
                      e.currentTarget.style.boxShadow =
                        "inset 0 1px 0 rgba(0,229,255,.1), 0 8px 32px rgba(0,0,0,.4)";
                    }}
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          HTTPS/TLS server active on port 8443
                        </span>
                      </div>
                      <div
                        style={{
              ...
              {tab === "Live Logs" && (
                <div className="space-y-2">
                  ...
                  {liveLogs.length === 0 && (
                    <div className="text-center text-text/30 text-xs py-8">
                      Waiting for events…
                    </div>
                  )}
                  {liveLogs.map((log, i) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center space-x-1.5">
                          <div
                            className={`w-1.5 h-1.5 rounded-full ${severityDot(log.severity, log.blocked)}`}
                          />
                          <span className="text-[10px] font-bold text-text/90 uppercase">
                            {log.attack_type}
                          </span>
                          {log.blocked && (
                            <span className="px-1 py-0.5 bg-low/20 text-low border border-low/20 rounded text-[8px] font-bold">
                              BLOCKED
                            </span>
                          )}
                        </div>
                      ...
                      <div className="text-[9px] font-mono text-text/50">
                        ...
                        {log.zone && (
                          <span className="ml-1 text-text/30">
                            [{log.zone}]
                          </span>
                        )}
                      </div>
                      <div className="text-[8px] text-text/30 mt-0.5">
                        ...
                        {log.detection_method && (
                          <span className="ml-1">{log.detection_method}</span>
                        )}
                      </div>
                    </div>
                  ))}
  ...
  <div className="page-footer">
    LightGuard IDS v3.0 · Detection Engine Active · Fernet Encrypted · Tadhamon
    Smart City— MEC 2025–2026
  </div>;
}
