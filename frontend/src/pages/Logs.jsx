...
export default function Logs({ user }) {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      setLogs(
        (data.items || []).map((l) => ({
          ...l,
          attack_type: clean(l.attack_type),
  ...
  const exportCSV = () => {
    ...
    const a = Object.assign(document.createElement("a"), {
      ...
      download: `lightguard_logs_${new Date().toISOString().slice(0, 10)}.csv`,
    });
    a.click();
  };
  ...
  const TYPES = [
    ...new Set(logs.map((l) => l.attack_type).filter(Boolean)),
  ].slice(0, 12);
    ...
    <div className="p-6 space-y-5 page-enter">
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 12, color: C.muted, marginTop: 4 }}>
            SOC-grade audit trail — {total.toLocaleString()} total events ·
            Tadhamon Smart City
          </p>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <HoverHint hint="Reload security logs from the database. New events appear at the top automatically.">
            <button
              onClick={fetchLogs}
              className="btn-cyber"
              style={{ height: 36, fontSize: 11, gap: 6 }}
              title="Reload logs from the database with current filters"
            >
          ...
          <HoverHint hint="Export all filtered log entries as a CSV file for offline analysis or SIEM integration.">
            <button
              ...
              className="btn-cyber"
              style={{ height: 36, fontSize: 11, gap: 6 }}
              title="Export filtered security logs to CSV for SIEM or reporting tools"
            >
        ...
        >
          {[
            ...
            },
            {
              label: "Zone",
              key: "zone",
              opts: [["", "All Zones"], ...ZONES.map((z) => [z, z])],
            },
            {
              label: "Attack Type",
              key: "attack_type",
            ...
            <div key={key}>
              ...
              >
                {opts.map(([v, l]) => (
                  <option key={v} value={v} style={{ background: "#071426" }}>
                    {l}
                  </option>
                ))}
              </select>
            ...
            <thead style={{ position: "sticky", top: 0, zIndex: 2 }}>
              ...
              >
                {[
                  {
                    h: "Timestamp",
                    tip: "Date and time the log entry was recorded.",
                  },
                  { h: "Source IP", tip: "Source IPv4 address of the event." },
                  { h: "Dest IP", tip: "Destination IPv4 address." },
                  {
                    h: "Protocol",
                    tip: "Network protocol: TCP, UDP, ICMP, or ARP.",
                  },
                  { h: "Port", tip: "Destination port number if available." },
                  { h: "Zone", tip: "VLAN zone where the event originated." },
                  {
                    h: "Device",
                    tip: "Device role or type assigned in the inventory.",
                  },
                  {
                    h: "Attack Type",
                    tip: "Attack classification by the detection engine.",
                  },
                  {
                    h: "Severity",
                    tip: "Severity level: CRITICAL, HIGH, MEDIUM, or LOW.",
                  },
                  {
                    h: "Detection",
                    tip: "Detection method: Live Detection Engine or Scenario Engine.",
                  },
                  {
                    h: "Action Taken",
                    tip: "Response recorded: Logged & Monitored, Alert Generated, or Under Review.",
                  },
                ].map(({ h, tip }) => (
                  <HoverHint
                    key={h}
            ...
            <tbody>
              ...
              {!loading &&
                logs.map((l, i) => {
                  ...
                  const actionColor = l.action_taken?.includes("Alert")
                    ? C.red
                    : l.action_taken?.includes("Monitoring")
                      ? C.orange
                      : C.muted;
                  return (
                    <tr
                    ...
                    >
                      ...
                      <td
                        className="mono-ip"
                        style={{ padding: "8px 12px", color: C.cyan }}
                      >
                        {l.source_ip || "—"}
                      </td>
                      <td
                        className="mono-ip"
                        style={{ padding: "8px 12px", color: "#85B7EB" }}
                      >
                        {l.destination_ip || "—"}
                      </td>
                      <td
                      ...
                      <td
                        className="mono-ip"
                        style={{ padding: "8px 12px", color: C.muted }}
                      >
                        {l.port || "—"}
                      </td>
                      <td
      ...
      >
        <span style={{ fontSize: 12, color: C.muted }}>
          Page {page} · showing {logs.length} of {total.toLocaleString()}
        </span>
        <div style={{ display: "flex", gap: 8 }}>
          <button
            ...
            className="btn-cyber"
            style={{ height: 32, fontSize: 10, opacity: page === 1 ? 0.4 : 1 }}
          >
            ← Prev
          </button>
          <button
