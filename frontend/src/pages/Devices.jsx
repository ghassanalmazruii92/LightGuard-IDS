...
const Devices = ({ user }) => {
  ...
  const getRiskBadge = (score) => {
    if (score <= 30)
      return (
        <span className="px-2 py-1 rounded-full text-[10px] font-bold bg-low/10 text-low uppercase">
          Low
        </span>
      );
    if (score <= 60)
      return (
        <span className="px-2 py-1 rounded-full text-[10px] font-bold bg-medium/10 text-medium uppercase">
          Medium
        </span>
      );
    if (score <= 80)
      return (
        <span className="px-2 py-1 rounded-full text-[10px] font-bold bg-high/10 text-high uppercase">
          High
        </span>
      );
    return (
      <span className="px-2 py-1 rounded-full text-[10px] font-bold bg-critical/10 text-critical uppercase animate-pulse">
        Critical
      </span>
    );
  };
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case "online":
        return (
          <div className="flex items-center space-x-1.5 text-low">
      ...
      case "suspicious":
        return (
          <div className="flex items-center space-x-1.5 text-high animate-pulse">
  ...
  const StatCard = ({ title, value, icon: Icon, color, hint }) => (
    ...
    >
      ...
      <div
        className={`w-12 h-12 rounded-lg flex items-center justify-center bg-${color}/10`}
      >
  ...
  const citySecurityScorePct = `${Math.max(0, Math.min(100, 100 - (Number(stats.avg_risk_score) || 0))).toFixed(1)}%`;
    ...
    <div className="page-enter space-y-6">
      <div className="flex items-center justify-between">
        ...
        >
          <div>
            <h1 className="text-2xl font-bold text-accent">
              Device Inventory – Tadhamon IoT Map
            </h1>
            <p className="text-text/60 text-sm mt-1">
              Monitoring {devices.length} city-wide connected assets
              {liveCount > 0 && (
                <span className="ml-2 text-low font-semibold">
                  {liveCount} live
                </span>
              )}
            </p>
        ...
        <div className="flex items-center space-x-2 shrink-0">
          <HoverHint
            hint="Reload device list and stats from the API."
            as="button"
            type="button"
            className="flex items-center space-x-2 px-3 py-2 bg-background border border-border rounded-lg text-text/60 hover:text-accent hover:border-accent/50 transition-all"
          ...
          {canManageDevices && (
            ...
            >
              <Search
                className={`w-4 h-4 ${scanningNetwork ? "animate-spin" : ""}`}
              />
        ...
        {[
          {
            ...
            hint: "Show every asset: discovered and seeded demo hosts.",
          },
          {
            key: "discovered",
            label: `Live (${liveCount})`,
            color: "text-low",
            hint: "Hosts seen by active discovery (ARP / scan) only.",
          },
          {
            key: "seed",
            label: `Demo (${demoCount})`,
            color: "text-text/40",
            hint: "Pre-seeded demo devices for presentation.",
          },
        ].map((tab) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          icon={Monitor}
          color="accent"
          hint="All rows in inventory (online + offline)."
        />
        <StatCard
          ...
          icon={Activity}
          color="low"
          hint="Devices currently marked online / reachable."
        />
        <StatCard
          ...
          icon={AlertTriangle}
          color="high"
          hint="Assets flagged suspicious or high-risk by scoring."
        />
        <StatCard
          title="City Security Score"
          value={citySecurityScorePct}
          icon={Shield}
          color="medium"
          hint="100% minus average portfolio risk score (higher is better)."
        />
      </div>
          ...
          <table className="w-full text-left">
            <thead className="bg-background/50 text-text/60 text-[10px] font-bold uppercase">
              <tr>
                <HoverHint
                  as="th"
                  hint="Zone icon, label, and whether this row is live-discovered or demo."
                ...
                <HoverHint
                  as="th"
                  hint="IPv4 address used for scans and API calls."
                  className="px-6 py-4 text-left font-bold"
                >
                  IP Address
                </HoverHint>
                <HoverHint
                  as="th"
                  hint="Asset role tag from inventory (e.g. sensor, gateway)."
                  className="px-6 py-4 text-left font-bold"
                >
                  Role
                </HoverHint>
                <HoverHint
                  as="th"
                  hint="Layer-2 identifier when known from discovery."
                  className="px-6 py-4 text-left font-bold"
                >
                  MAC Address
                </HoverHint>
                <HoverHint
                  as="th"
                  hint="Heuristic risk score from telemetry and scans."
                  className="px-6 py-4 text-left font-bold"
                >
                  Risk Level
                </HoverHint>
                <HoverHint
                  as="th"
                  hint="Reachability / suspicion state."
                  className="px-6 py-4 text-left font-bold"
                >
                  Status
                </HoverHint>
                <HoverHint
                  as="th"
                  hint="Admin-only: deep scan and trust toggle."
            ...
            <tbody className="divide-y divide-border">
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-3">
                        <div
                          className={`p-2 rounded-lg bg-background border transition-all group-hover:border-accent/30 ${isLive ? "border-low/40" : "border-border"} ${device.risk_score > 60 ? "border-high/30" : ""}`}
                        >
                          <ZoneIcon
                            className={`w-5 h-5 ${device.risk_score > 60 ? "text-high" : isLive ? "text-low" : "text-accent"}`}
                          />
                        </div>
                        <div>
                          <div className="text-sm font-bold text-text flex items-center space-x-1.5">
                            ...
                            {device.trusted && (
                              <ShieldCheck className="w-3 h-3 text-low" />
                            )}
                            {isLive ? (
                              ...
                              <span className="px-1.5 py-0.5 bg-background text-text/30 border border-border rounded text-[9px] font-bold">
                                DEMO
                              </span>
                            )}
                          </div>
                    ...
                    <td className="px-6 py-4 text-xs font-mono text-accent">
                      {device.ip}
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-[10px] font-bold text-text/60 bg-background border border-border px-2 py-0.5 rounded uppercase">
                        {device.role || "unknown"}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-[10px] text-text/40 font-mono">
                      {device.mac}
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end space-x-2">
                        <HoverHint
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            runDeepScan(device.ip);
                          }}
                          disabled={
                            scanningIp === device.ip || !canManageDevices
                          }
                        >
                          <Search
                            className={`w-3.5 h-3.5 ${scanningIp === device.ip ? "animate-spin" : ""}`}
                          />
                        </HoverHint>
                        <HoverHint
                          as="button"
                          ...
                          hint={
                            device.trusted
                              ? "Remove trusted override for this asset."
                              : "Mark this asset as trusted to reduce alert noise."
                          }
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            toggleTrust(device.ip, device.trusted);
                          }}
                          disabled={
                            trustingIp === device.ip || !canManageDevices
                          }
                        >
                          <ShieldCheck
                            className={`w-3.5 h-3.5 ${trustingIp === device.ip ? "animate-pulse" : ""}`}
                          />
                        </HoverHint>
                      </div>
                    </td>
              ...
              {filteredDevices.length === 0 && (
                <tr>
                  <td
                    colSpan="7"
                    className="px-6 py-12 text-center text-text/40 italic"
                  >
                    {loading
      ...
      {addOpen && user && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-background/80 backdrop-blur-md">
          <div className="bg-card border border-border rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-text/50 mt-0.5">
                    IP address must fall within the selected VLAN subnet
                  </p>
                </div>
              </div>
              <button
            ...
            >
              {addError && (
                <div className="px-3 py-2 rounded-lg bg-high/10 border border-high/30 text-high text-sm">
                  {addError}
                </div>
              )}
              <div>
                <label className="block text-[10px] font-bold text-text/40 uppercase mb-1.5">
                  Zone
                </label>
                <select
                  required
                  ...
                  value={addForm.zone}
                  onChange={(e) =>
                    setAddForm((f) => ({ ...f, zone: e.target.value }))
                  }
                >
              ...
              <div>
                ...
                <input
                  ...
                  value={addForm.ip}
                  onChange={(e) =>
                    setAddForm((f) => ({ ...f, ip: e.target.value }))
                  }
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAddForm((f) => ({ ...f, hostname: e.target.value }))
                    }
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    value={addForm.mac}
                    onChange={(e) =>
                      setAddForm((f) => ({ ...f, mac: e.target.value }))
                    }
                  />
                </div>
                <div>
              ...
              <div>
                ...
                <input
                  ...
                  value={addForm.os}
                  onChange={(e) =>
                    setAddForm((f) => ({ ...f, os: e.target.value }))
                  }
                />
              </div>
          ...
          <div className="bg-card border border-border rounded-3xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl animate-in fade-in zoom-in duration-300">
            ...
            <div className="p-8 border-b border-border flex items-center justify-between bg-background/50">
              <div className="flex items-center space-x-5">
                <div
                  className={`p-4 rounded-2xl bg-${selectedDevice.device?.status === "suspicious" ? "high" : "accent"}/10 border border-${selectedDevice.device?.status === "suspicious" ? "high" : "accent"}/20`}
                >
                  {selectedDevice.device?.zone ? (
                    React.createElement(
                      zoneIcons[selectedDevice.device.zone] || Monitor,
                      {
                        className: `w-8 h-8 text-${selectedDevice.device?.status === "suspicious" ? "high" : "accent"}`,
                      },
                    )
                  ) : (
                ...
                <div>
                  <div className="flex items-center space-x-2 flex-wrap gap-1">
                    <h2 className="text-2xl font-bold">
                      {selectedDevice.device?.label ||
                        selectedDevice.device?.hostname}
                    </h2>
                    {selectedDevice.device?.trusted && (
                      <div className="px-2 py-0.5 bg-low/10 text-low border border-low/20 text-[10px] font-bold rounded uppercase">
                        Trusted Asset
                      </div>
                    )}
                    {selectedDevice.device?.source === "discovered" ? (
                      ...
                      <div className="px-2 py-0.5 bg-accent/10 text-accent border border-accent/30 text-[10px] font-bold rounded uppercase">
                        Manual
                      </div>
                    ) : (
                      <div className="px-2 py-0.5 bg-background text-text/30 border border-border text-[10px] font-bold rounded uppercase">
                        Demo
                      </div>
                    )}
                  </div>
                  <div className="flex items-center space-x-3 text-text/60 text-sm mt-1 font-mono">
                    <span className="flex items-center space-x-1">
                      ...
                      <span>
                        {selectedDevice.device?.zone || "Unassigned Zone"}
                      </span>
                    </span>
                    <span>•</span>
              ...
              <div className="space-y-8">
                <section>
                  ...
                  <div className="p-6 bg-background rounded-2xl border border-border flex flex-col items-center justify-center text-center">
                    <div
                      className={`text-5xl font-bold mb-1 ${selectedDevice.device?.risk_score > 60 ? "text-high" : "text-low"}`}
                    >
                      {selectedDevice.device?.risk_score}%
                    </div>
                    <div className="text-[10px] font-bold text-text/40 uppercase">
                      Total Threat Index
                    </div>
                    <div className="w-full h-1.5 bg-border rounded-full mt-4 overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${selectedDevice.device?.risk_score}%`,
                        }}
                      />
                    </div>
                  </div>
                ...
                <section>
                  <h3 className="text-[10px] font-bold text-text/40 uppercase mb-4">
                    Device Metadata
                  </h3>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center py-2 border-b border-border/50">
                      ...
                      <span className="text-xs font-bold text-accent uppercase">
                        {selectedDevice.device?.role || "Unknown"}
                      </span>
                    </div>
                    <div className="flex justify-between items-center py-2 border-b border-border/50">
                      ...
                      <span className="text-xs font-medium">
                        {selectedDevice.device?.os || "Undetected"}
                      </span>
                    </div>
                    <div className="flex justify-between items-center py-2 border-b border-border/50">
                      <span className="text-xs text-text/60">
                      ...
                      <span className="text-xs font-medium">
                        {new Date(
                          selectedDevice.device?.first_seen,
                    ...
                    <div className="flex justify-between items-center py-2">
                      ...
                      <span className="text-xs font-medium text-low">
                        {new Date(
                          selectedDevice.device?.last_seen,
                ...
                {canManageDevices && (
                  <section className="pt-4 border-t border-border">
                    ...
                    >
                      <Search
                        className={`w-4 h-4 ${scanningIp === selectedDevice.device?.ip ? "animate-spin" : ""}`}
                      />
                      <span>
                        {scanningIp === selectedDevice.device?.ip
                          ? "Running Deep Scan..."
                  ...
                  <section className="bg-background/30 p-6 rounded-2xl border border-border">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedDevice.device?.open_ports?.map((port) => (
                        ...
                        <div className="text-xs text-text/30 italic py-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <section className="bg-background/30 p-6 rounded-2xl border border-border">
                    ...
                    <div className="space-y-3">
                      {selectedDevice.device?.vulnerabilities?.map(
                        (vuln, i) => (
                          <div
                            key={i}
                            className="p-3 bg-high/5 rounded-xl border border-high/20"
                          >
                            <div className="text-[10px] font-bold text-high uppercase flex justify-between items-center mb-1">
                              ...
                              <div className="flex items-center space-x-1">
                                {vuln.cve && (
                                  <span className="px-1.5 py-0.5 bg-high/20 text-high border border-high/30 rounded text-[9px]">
                                    {vuln.cve}
                                  </span>
                                )}
                                <span
                                  className={`px-1.5 py-0.5 text-white rounded text-[9px] ${vuln.severity === "CRITICAL" ? "bg-critical" : "bg-high"}`}
                                >
                                  {vuln.severity || "VULN"}
                                </span>
                              </div>
                            </div>
                            <p className="text-[10px] text-high/80 leading-relaxed mb-2">
                              {vuln.summary || vuln.output}
                            </p>
                            {vuln.recommendation && (
                        ...
                        <div className="text-xs text-text/30 italic py-4 flex items-center space-x-2">
                  ...
                  <section className="bg-background/30 p-6 rounded-2xl border border-border">
                    ...
                    <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                      {selectedDevice.alerts?.map((alert) => (
                        <div
                          key={alert.id}
                          className="p-3 bg-background rounded-xl border border-border/50 flex items-center justify-between transition-all hover:bg-background/50"
                        >
                          <div className="flex items-center space-x-4">
                            <div
                              className={`w-2.5 h-2.5 rounded-full ${alert.severity === "CRITICAL" ? "bg-critical" : alert.severity === "HIGH" ? "bg-high" : "bg-medium"}`}
                            />
                            <div>
                              <p className="text-[10px] font-bold uppercase text-text/90">
                                {alert.attack_type}
                              </p>
                              <p className="text-[9px] text-text/40 mt-0.5">
                          ...
                          <span className="text-[8px] font-bold text-text/30 border border-border px-1.5 py-0.5 rounded uppercase">
                            {alert.detection_method}
                          </span>
                        </div>
                      )) || (
                        <div className="text-xs text-text/30 italic py-8 text-center bg-background rounded-2xl border border-border/30">
                          Zero active intrusion records.
                        </div>
                      )}
                    </div>
                  </section>
