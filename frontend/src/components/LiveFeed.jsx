...
const LiveFeed = ({ alerts }) => {
  ...
  const getMethodBadge = (method) => {
    const badges = {
      "arp-scan": {
        color: "bg-low/20 text-low border-low/30",
        label: "ARP",
        icon: Monitor,
      },
      masscan: {
        color: "bg-high/20 text-high border-high/30",
        label: "MASSCAN",
        icon: Zap,
      },
      "nmap-vuln-scan": {
    ...
    const badge = badges[method] || {
      color: "bg-text/10 text-text/60 border-text/20",
      label: method,
      icon: Shield,
    };
    ...
    return (
      <span
        className={`flex items-center space-x-1 px-1.5 py-0.5 rounded border text-[8px] font-bold uppercase ${badge.color}`}
      >
        ...
        {alerts.map((alert) => (
          ...
          >
            <div className="flex items-start space-x-3 p-3 bg-background/50 rounded-lg border border-border/50 group hover:border-accent/30 transition-all hover:translate-x-1 cursor-default">
              <div
                className={`w-2 h-2 mt-1.5 rounded-full shrink-0 ${getSeverityColor(alert.severity)}`}
              />
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm font-bold text-accent truncate uppercase tracking-tight">
                    {alert.attack_type}
                  </span>
                  <span className="text-[10px] text-text/40 shrink-0">
                ...
                <div className="text-xs text-text/70 truncate flex items-center justify-between gap-2">
                  <span>
                    {alert.src_ip} <span className="text-text/30">→</span>{" "}
                    {alert.dst_ip}
                  </span>
