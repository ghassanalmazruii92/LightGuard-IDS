...
## How to run everything in this app
...
This launches the backend and dashboard server on:
...
- seeds default users and demo data
- starts the IDS capture/log observer
- starts the mock network scanner
...
```powershell
cd C:\Users\Ghass\lightguard11\frontend
npm run dev
```
...
Vite will proxy API and WebSocket traffic to the backend.
...
## If you want to run everything from one server with the built frontend
...
The backend can serve the built frontend from static.
...
If you have the GNS3 network running:
...
That is the full setup to run the app and the dashboard.
